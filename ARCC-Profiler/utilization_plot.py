...
def create_plots_plot(events, keys, height, width, event_interval, no_legend, save_name, cpu_events, sub_plot_flag,
    ...
    if split_gpus is True:
        ...
        create_plot(gpu_timestamp_dictionary, gpu_utilization_dictionary, keys, cpu_timestamps, cpu_utilization,
                    sub_plot_item, height, width, event_interval, no_legend, save_name, sub_plot_flag, x_label, y_label,
                    plot_title)
        y_label = "Power Used (Watts)"
        plot_title = "GPU Power Utilization"
...
def GPU_Plot_Runtime(interval=25, height=5, width=10, combine_graphs=False):
    ...
    cl_parser.add_argument('-H', '--height', type=float, default=height, help='Height for generated plots. '
                                                                              'Default is 5 inches.')
    cl_parser.add_argument('-W', '--width', type=float, default=width, help='Width for generated plots. '
                                                                            'Default is 10 inches.')
    cl_parser.add_argument('-i', '--interval', type=int, default=interval,
    ...
    cl_parser.add_argument('-s', '--split', action='store_true', default=combine_graphs, help='Split multiple GPUs '
                                                                                              'out into separate '
                                                                                              'plots.')
